import { ensureTypeOf_sheet, PAYMENTTYPE_SHEET_TITLE } from '../_lib/sheets.js';
    ...
    if (req.method === 'GET') {
        ...
        try {
            ...
            const sheetsRes = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${PAYMENTTYPE_SHEET_TITLE}!A:C`,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            );
            const data = await sheetsRes.json();
            if (!sheetsRes.ok) {
                return res.status(sheetsRes.status).json({ error: 'Failed to fetch payment_type', detail: data });
            }

            const rows = data.values || [];
            const [header, ...body] = rows;
            if (!header) return res.status(200).json({ payment_type: [] });

            const payment_type = body.map((row) => {
                const obj = {};
            ...
            console.error('payment_type fetch error:', err);
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const appendRes = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${PAYMENTTYPE_SHEET_TITLE}!A:C:append?valueInputOption=USER_ENTERED`,
                {
                    method: 'POST',
                    headers: {
